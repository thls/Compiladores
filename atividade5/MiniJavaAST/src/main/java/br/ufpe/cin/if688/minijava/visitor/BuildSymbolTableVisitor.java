...
public class BuildSymbolTableVisitor implements IVisitor<Void> {
	...
	public Void visit(ClassDeclSimple n) {
		...
		for (int i = 0; i < n.ml.size(); i++) {
			...
			if(!(currClass.addMethod(currentMethod.i.toString(), currentMethod.t))){
				PrintException.duplicateMethod(currentMethod.i.toString());
			}
			currMethod = symbolTable.getMethod(currentMethod.i.toString(), currClass.getId());
